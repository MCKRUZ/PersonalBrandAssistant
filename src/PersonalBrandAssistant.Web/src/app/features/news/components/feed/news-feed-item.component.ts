...
@Component({
    ...
    <article class="feed-card" [style.--source-color]="categoryColor()">
      ...
      <div class="feed-card__body">
        ...
        <div class="feed-card__meta">
          ...
          <span class="feed-card__source-name">{{ item().sourceName ?? item().source }}</span>
          <span class="feed-card__separator">&middot;</span>
          <span class="feed-card__time">{{ item().createdAt | relativeTime }}</span>
          <div class="feed-card__score" [attr.data-level]="scoreLevel()">
        ...
        <div class="feed-card__actions">
          ...
          @if (!item().summary && item().url) {
            <button class="feed-card__action feed-card__action--analyze" [disabled]="analyzing()" (click)="analyzed.emit()">
              @if (analyzing()) {
  ...
  styles: `
    ...
    .feed-card__score[data-level="high"] { background: #22c55e; box-shadow: 0 0 12px rgba(34, 197, 94, 0.3); }
    .feed-card__score[data-level="medium"] { background: #eab308; box-shadow: 0 0 12px rgba(234, 179, 8, 0.3); }
    .feed-card__score[data-level="low"] { background: #ef4444; box-shadow: 0 0 12px rgba(239, 68, 68, 0.3); }

...
@Component({
  ...
  imports: [FormsModule, Select, InputTextModule, IconFieldModule, InputIconModule, BadgeModule, ButtonModule, Tooltip],
  template: `
    <div class="filter-bar">
      ...
      <p-button
        [icon]="store.groupMode() === 'category' ? 'pi pi-th-large' : 'pi pi-list'"
        [text]="true" size="small"
        [style]="{ color: 'white' }"
        [pTooltip]="store.groupMode() === 'category' ? 'Group by source' : 'Group by category'"
        (onClick)="store.toggleGroupMode()"
      />
  ...
  styles: `
    ...
    @media (max-width: 768px) {
      ...
      .filter-bar__sources::-webkit-scrollbar { display: none; }
      .source-chip {
        flex-shrink: 0;

...
import { TopicCluster, SOURCE_COLORS, CATEGORY_COLORS } from '../../models/news.model';

@Component({
  selector: 'app-topic-cluster-card',
  standalone: true,
  encapsulation: ViewEncapsulation.None,
  imports: [ButtonModule, BadgeModule, VelocityIndicatorComponent],
  template: `
    <article
      class="cluster-card"
      ...
      <div class="cluster-card__articles">
        @for (article of topArticles(); track article.id) {
          <a
            ...
            [style.--article-color]="getCategoryColor(article.sourceCategory)"
          >
  ...
  styles: `
    ...
    @media (max-width: 768px) {
      ...
      .cluster-card__title { font-size: 0.95rem; }
      .cluster-card.featured .cluster-card__title { font-size: 1.1rem; }
      .cluster-card__rationale { font-size: 0.8rem; }
      .cluster-card__article { font-size: 0.78rem; padding: 0.35rem 0.4rem; }
      .cluster-card:hover { transform: none; }
    }
...
export class TopicClusterCardComponent {
  ...
  readonly heatIntensity = computed(() => Math.min(this.cluster().heat / 15, 1));

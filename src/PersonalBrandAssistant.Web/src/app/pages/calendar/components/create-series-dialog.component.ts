...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, Dialog, InputTextModule, TextareaModule, Select, MultiSelect, DatePicker, Button],
  template: `
...
export class CreateSeriesDialogComponent {
  ...
  onSubmit() {
    ...
    this.calendarApi.createSeries({
      ...
      error: () => {
        this.saving = false;
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to create series' });
      },
    });
  }
}

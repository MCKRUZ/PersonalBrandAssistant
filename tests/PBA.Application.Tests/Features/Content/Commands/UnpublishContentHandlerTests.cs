...
public class UnpublishContentHandlerTests
{
    ...
    [Fact]
    public async Task Handle_FromPublished_TransitionsToDraft()
    {
        await using var context = CreateContext();
        var content = new ContentEntity { Title = "Test", Body = "Text", Status = ContentStatus.Published };
        context.Contents.Add(content);
        await context.SaveChangesAsync();

        var handler = new UnpublishContent.Handler(context);
        var result = await handler.Handle(new UnpublishContent.Command(content.Id), CancellationToken.None);
    ...
    public async Task Handle_NonexistentContent_ReturnsNotFound()
    {
        ...
        var handler = new UnpublishContent.Handler(context);
        var result = await handler.Handle(new UnpublishContent.Command(Guid.NewGuid()), CancellationToken.None);

...
public class SidecarClientTests : IAsyncDisposable
{
    ...
    public async Task SendTaskAsync_StreamsEventsAsIAsyncEnumerable()
    {
        ...
        await foreach (var evt in client.SendTaskAsync("do something", null, null, null, CancellationToken.None))
        {
            events.Add(evt);
        }
    ...
    public async Task SendTaskAsync_YieldsTaskCompleteEvent_WithTokenCounts()
    {
        ...
        await foreach (var evt in client.SendTaskAsync("test", null, null, null, CancellationToken.None))
        {
            lastEvent = evt;
        }
    ...
    public async Task SendTaskAsync_CancelsOnCancellationToken()
    {
        ...
        try
        {
            await foreach (var evt in client.SendTaskAsync("test", null, null, null, cts.Token))
            {
                events.Add(evt);
                if (events.Count >= 2)
                    cts.Cancel();
